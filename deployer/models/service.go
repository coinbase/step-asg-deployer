...
import (
	...
	"github.com/coinbase/step-asg-deployer/aws/lc"
...
func (service *Service) SetDefaults(release *Release, serviceName string) {
	...
	if release.ProjectName != nil && release.ConfigName != nil {
		service.ServiceID = to.TimeUUID(fmt.Sprintf("%v-%v-%v-", *release.ProjectName, *release.ConfigName, *service.ServiceName))
	}
...
func (service *Service) ValidateAttributes() error {
	...
	if is.EmptyStr(service.ServiceID) {
		return fmt.Errorf("ServiceID must be defined")
	}
...
func (service *Service) createInput() *asg.Input {
	...
	input.AddTag("Name", to.Strp(fmt.Sprintf("%v-%v-%v-%v", *service.ProjectName(), *service.ConfigName(), *service.ServiceName, *service.ReleaseID())))
